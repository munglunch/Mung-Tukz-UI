...
if (not oUF.Tags['[DiffColor]']) then
	oUF.Tags['[DiffColor]']  = function(unit)
		...
		if (level < 1) then
			...
			if (DiffColor >= 5) then
				r, g, b = 0.93,0.07,0.07
			elseif (DiffColor >= 3) then
				r, g, b = 0.81, 0.43, 0.17
			elseif (DiffColor >= -2) then
				r, g, b = 0.84, 0.75, 0.65
			elseif (-DiffColor <= GetQuestGreenRange()) then
				r, g, b = 0.13, 0.89, 0.13
			else
...
oUF.TagEvents['[NameTiny]'] = 'UNIT_NAME_UPDATE'
if (not oUF.Tags['[NameTiny]']) then
	oUF.Tags['[NameTiny]'] = function(unit)
