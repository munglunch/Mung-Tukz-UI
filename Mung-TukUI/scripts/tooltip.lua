...
for i=1, #tooltips do
		_G[tooltips[i]]:SetScript("OnShow", function(self) TukuiDB:SetTemplate(self,2) end)
--		TukuiDB:Nglow(_G[tooltips[i]])
end
...
GameTooltipStatusBar:SetPoint("TOPLEFT", GameTooltip, "BOTTOMLEFT", TukuiDB:Scale(5), TukuiDB:Scale(2))
GameTooltipStatusBar:SetPoint("TOPRIGHT", GameTooltip, "BOTTOMRIGHT", TukuiDB:Scale(-5), TukuiDB:Scale(2))
...
local function defaultPosition(tt, parent)
	if db.cursor == true then
		...
		tt:SetPoint("BOTTOMLEFT", AlertChat, "TOPLEFT", 0, TukuiDB:Scale(10))
	end
end
...
local function gtUpdate(self, ...)
	if self:GetAnchorType() == "ANCHOR_NONE" then
		if TukuiDB["bags"].enable == true and StuffingFrameBags:IsShown() then
			self:ClearAllPoints()
			self:SetPoint("TOPRIGHT", StuffingFrameBags, "TOPLEFT", TukuiDB:Scale(-4), 0)
		else
			self:ClearAllPoints()
			self:SetPoint("BOTTOMLEFT", AlertChat, "TOPLEFT", 0, TukuiDB:Scale(10))
		end
	end
end
