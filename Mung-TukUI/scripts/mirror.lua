...
local _DEFAULTS = {
	...
	texture = TukuiDB["media"].castb2,
	texture2 = TukuiDB["media"].blank,
	position = {
...
do
	...
	function Spawn(type)
		...
		local stitched = ((settings.height * 2) / 4)
		local bg = frame:CreateTexture(nil, 'BACKGROUND')
		bg:SetSize(settings.width, settings.height)
		bg:SetPoint("CENTER", frame)
		bg:SetTexture(settings.texture2)
		bg:SetVertexColor(r * .1, g * .1, b * .1)
		
		local border = CreateFrame("Frame", nil, frame)
		border:SetPoint("TOPLEFT", frame, TukuiDB:Scale(-2), (-1 * stitched))
		border:SetPoint("BOTTOMRIGHT", frame, TukuiDB:Scale(2), stitched)
		TukuiDB:SetTemplate(border)
		border:SetFrameLevel(0)

		local text = frame:CreateFontString(nil, 'OVERLAY')
		text:SetFont(TukuiDB["media"].superfont, 18, "THICKOUTLINE")
		...
		frame:SetSize(settings.width, (settings.height * 2))
